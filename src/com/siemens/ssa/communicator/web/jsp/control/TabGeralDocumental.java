 ...
 *
 */
public class TabGeralDocumental extends TabGeralDocumentalGeneric {
	/**
	 * Serial UID
	 */
	...
	public TabGeralDocumental(DgitaLayoutPage tbControl, boolean readOnly, String control, DeclarationProcessor declarationProcessor) {
		
		PhyControlDoc = new Tab("PhyControlDoc");
		identControlo=control;
		log.info("TabGeralDocumental#idControlo:"+identControlo);
		//docsTable.setTableTitle("Documentos da adi&ccedil;&atilde;o");
		docsTable.setAttribute("align", "center");

		ControlFieldColumn col1 = new ControlFieldColumn("documento");
		col1.setHeaderTitle("Documento");
		...
		if(!readOnly){
			confereDoc.setOnClick("marcarTodos('.checkbox-adicoes'); return false;");
			docsTable.addButtonToTable(confereDoc, tbControl.getForm().getName());
		}
		int[] flds = {1};
		PhyControlDoc.setNumberFieldsPerLine(flds);
		if(declarationProcessor.getWidgetVisible(docsTable.getId())) PhyControlDoc.addField(docsTable);
	...
	public void setFormulario(Controlo controlo, Form form){
		...
		if(controlo.getListaControloItem()!=null){
			...
			for(int x=0;x<controlo.getListaControloItem().size(); x++){
				...
				for(int i=0;i<controloAdd.getListaControloDocumentoItem().size();i++){
					
					ControloDocumento ctrDocAdi = controloAdd.getListaControloDocumentoItem().get(i);
